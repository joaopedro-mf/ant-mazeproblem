...
def view_path_construction(path, occupancy_map, initial_node, final_node):
...
def view_path(path, occupancy_map, initial_node, final_node):
    ''' Represents the path in the map '''
    x = []
    y = []
    for p in path:
...
if __name__ == '__main__':
    ...
    try: 
        ...
        while(it < params["itt"]):
            ...
            if(params["disp"]):
                view_path_construction(real_path, full_map.occupancy_grid, full_map.initial_node, full_map.final_node)
    ...
    view_path(real_path, full_map.occupancy_grid, full_map.initial_node, full_map.final_node)
